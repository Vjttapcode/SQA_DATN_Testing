...
//            academicYear = AcademicYearUtils.resolveAcademicYear(academicYear);
    ...
    @Operation(summary = "Lấy môn học theo ID", description = "Lấy thông tin chi tiết môn học theo ID")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Thành công")
    ...
    public ResponseEntity<ApiResponse<SubjectResponse>> getSubjectById(@PathVariable Long id) {
        try {
            ...
            ApiResponse<SubjectResponse> response = ApiResponse.success(subject, "Lấy môn học thành công");
            return ResponseEntity.ok(response);
        } catch (RuntimeException e) {
