...
public interface SubjectRepository extends JpaRepository<Subject, Long> {
     ...
     */
    @Query("SELECT s FROM Subject s JOIN s.semester sem WHERE s.subjectCode = :subjectCode AND sem.semesterName = :semesterName AND sem.academicYear = :academicYear ORDER BY s.id")
     ...
     */
    @Query("SELECT s FROM Subject s JOIN s.major m JOIN s.semester sem WHERE s.subjectCode = :subjectCode AND m.majorCode = :majorCode AND sem.semesterName = :semesterName AND sem.academicYear = :academicYear AND m.classYear = :classYear")
    ...
    @Query("SELECT new com.ptit.schedule.dto.SubjectMajorDTO(s.subjectCode, s.subjectName, m.majorCode, m.classYear, s.theoryHours, s.exerciseHours, s.labHours, s.projectHours, s.selfStudyHours, m.numberOfStudents, s.studentsPerClass) FROM Subject s JOIN s.major m")
    List<SubjectMajorDTO> getAllSubjectsWithMajorInfo();

    // Lấy danh sách subject kèm thông tin ngành
    @Query("SELECT new com.ptit.schedule.dto.SubjectMajorDTO(s.subjectCode, s.subjectName, m.majorCode, m.classYear, s.theoryHours, s.exerciseHours, s.labHours, s.projectHours, s.selfStudyHours, m.numberOfStudents, s.studentsPerClass) FROM Subject s JOIN s.major m WHERE s.semester.semesterName = :semesterName AND s.semester.academicYear = :academicYear AND m.classYear = :classYear AND s.programType = :programType AND s.isCommon = false")
    ...
    @Query("SELECT new com.ptit.schedule.dto.SubjectMajorDTO(s.subjectCode, s.subjectName, m.majorCode, m.classYear, s.theoryHours, s.exerciseHours, s.labHours, s.projectHours, s.selfStudyHours, m.numberOfStudents, s.studentsPerClass) FROM Subject s JOIN s.major m WHERE s.semester.semesterName = :semesterName AND s.semester.academicYear = :academicYear AND m.classYear = :classYear AND s.programType = :programType AND m.majorCode IN :majorCodes AND s.isCommon = false")
     ...
     */
    @Query("SELECT new com.ptit.schedule.dto.SubjectMajorDTO(s.subjectCode, s.subjectName, m.majorCode, m.classYear, s.theoryHours, s.exerciseHours, s.labHours, s.projectHours, s.selfStudyHours, CAST(SUM(m.numberOfStudents) AS integer), s.studentsPerClass) FROM Subject s JOIN s.major m WHERE s.semester.semesterName = :semesterName AND s.semester.academicYear = :academicYear AND s.isCommon = true GROUP BY s.subjectCode, s.subjectName, m.majorCode, m.classYear, s.theoryHours, s.exerciseHours, s.labHours, s.projectHours, s.selfStudyHours, s.studentsPerClass")
     ...
     */
    @Query("SELECT s FROM Subject s JOIN s.major m LEFT JOIN s.semester sem WHERE (:academicYear IS NULL OR sem.academicYear = :academicYear) AND (:semesterName IS NULL OR sem.semesterName = :semesterName) AND (:classYear IS NULL OR m.classYear = :classYear) AND (:majorCode IS NULL OR m.majorCode = :majorCode) AND (:programType IS NULL OR s.programType = :programType)")
